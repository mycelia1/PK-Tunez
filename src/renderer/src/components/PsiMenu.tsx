        ...
        <div className="psi-menu__body">
          <div className="psi-menu__grid">
            ...
            <label className="psi-menu__field">
              ...
              <small className="psi-menu__help">
                Randomized delay range for SoundCloud and YouTube downloads.
              </small>
            </label>
            ...
            <label className="psi-menu__field">
              ...
              />
              <small className="psi-menu__help">
                Spaces out metadata/API requests. 0 disables. Helps SoundCloud throttling; also applies to YouTube.
              </small>
            </label>
            ...
            <details className="psi-menu__section">
              <summary className="psi-menu__section-summary eb-label">SoundCloud settings</summary>
              <div className="psi-menu__section-body">
                <label className="psi-menu__field">
                ...
                <label className="psi-menu__field">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      onChange({ chunkSize: Math.max(0, Math.floor(Number(event.target.value) || 0)) })
                    }
                  />
            ...
            <details className="psi-menu__section">
              <summary className="psi-menu__section-summary eb-label">Spotify settings</summary>
              <div className="psi-menu__section-body">
                <p className="psi-menu__help">
                  Paste an open.spotify.com track, playlist, album, or artist link. Audio is matched from YouTube (and
                  similar sources) via spotDL — Spotify itself is not downloaded.
                </p>
                <label className="psi-menu__field">
                  <span className="eb-label">Client ID</span>
                  <input
                    ...
                    placeholder="Optional Spotify developer Client ID"
                  />
                </label>
                <label className="psi-menu__field">
                  <span className="eb-label">Client Secret</span>
                  <input
                    className="eb-input"
                    value={settings.spotifyClientSecret}
                    onChange={(event) => onChange({ spotifyClientSecret: event.target.value })}
                    placeholder="Optional Spotify developer Client Secret"
                    type="password"
                  />
                </label>
                <small className="psi-menu__help">
                  Leave blank to use the keyless matcher. Provide a Spotify developer Client ID and Secret only to
                  download private playlists or liked songs.
                </small>
              </div>
            </details>

            <details className="psi-menu__section">
              <summary className="psi-menu__section-summary eb-label">YouTube settings</summary>
              <div className="psi-menu__section-body">
                <label className="psi-menu__toggle">
                  <input
                    type="checkbox"
                    checked={settings.youtubeCookiesFromBrowser}
                    onChange={(event) => onChange({ youtubeCookiesFromBrowser: event.target.checked })}
                  />
                  <span>Use cookies from browser</span>
                  <small className="psi-menu__help">
                    Passes your logged-in YouTube session to yt-dlp for age-restricted, private, or similar videos.
                    Sign in to YouTube in the browser first, confirm the video plays there, then enable this.
                  </small>
                </label>

                <label className="psi-menu__field">
                  <span className="eb-label">Browser for cookies</span>
                  <select
                    className="eb-input"
                    value={settings.youtubeCookiesBrowser}
                    disabled={!settings.youtubeCookiesFromBrowser}
                    onChange={(event) =>
                      onChange({ youtubeCookiesBrowser: event.target.value as YouTubeCookiesBrowser })
                    }
                  >
